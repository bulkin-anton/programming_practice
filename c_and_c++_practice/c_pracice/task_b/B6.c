...
{
    /// WRITE HERE
    int state = 0, mas_el1, mas_el2, mas_el3, next_seq = 1, i = 0;
    if (fsm_size != 0){
        for (unsigned k = 0; k < fsm_size - 1;  k++){
            for (unsigned j = (fsm_size - 1); j > k; j--){
                if (fsm[3 * j - 3] > fsm [3 * j]){
                    mas_el1 = fsm[3 * (j - 1)];
                    mas_el2 = fsm[3 * (j - 1) + 1];
                    mas_el3 = fsm[3 * (j - 1) + 2];
                    fsm[3 * (j - 1)] = fsm[3 * j];
                    fsm[3 * (j - 1) + 1] = fsm[3 * j + 1];
                    fsm[3 * (j - 1) + 2] = fsm[3 * j + 2];
                    fsm[3 * j] = mas_el1;
                    fsm[3 * j + 1] = mas_el2;
                    fsm[3 * j + 2] = mas_el3;
                }
    ...
    printf("int\nmain(void){\n");
    printf("    int state = 0, metka, ans_no = 0;\n");
    printf("    scanf(\"%%d\", &metka);\n");
    printf("    while(metka != 0){\n");
    if (fsm_size != 0){
        printf("        switch(state){\n");
        while (i < fsm_size){
            if (next_seq == 1){
            ...
            if (state == fsm[3*i]){
                next_seq = 0;
                printf("                    case %d: state = %d; break;\n",
                fsm[3 * i + 1], fsm[3 * i + 2]); 
            }
            else{
                next_seq = 1;
                printf("                    default: ans_no = 1;\n");
                printf("                }\n");
                printf("                break;\n");
                state = fsm[3*i];
                i--;
