...
#define XABRT(e) ({ if (e) { fprintf(stderr, "Failed function\n"); abort();} })
...
#define NEWVECTORITERATOR(v) ({ VectorIterator *it = malloc(sizeof *it); it->ti = (struct functions){.__remove = VectorIterator__remove, .__same = VectorIterator__same, .__get = VectorIterator__get, .__next = VectorIterator__next }; it->ptr = (v); it; })
...
int Vector__end(Vector *v, void **i) {
    if (v->sz == 0) {
        *i = NEWVECTORITERATOR(0);
    } else {
        *i = NEWVECTORITERATOR(v->nums + v->sz);
    }
    return 0;
}
